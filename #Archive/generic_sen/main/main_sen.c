...
RTC_DATA_ATTR static uint32_t rtc_sleep_time_ms;
...
printf("from: %02x | 0x%08x | SNR: %02d | T: %dms\n",
	pHdr->mac_from[5], pHdr->frameid, pRx_ctrl->rssi - pRx_ctrl->noise_floor, tix);
		...
		if ((actual_frame_id == pHdr->frameid) && (pHdr->vtype == ACK_FROM_GW)) {
			 acked_frame_id = pHdr->frameid;
		}
...
timer = esp_timer_get_time();
	...
	return actual_frame_id != tx_frame.wiog_hdr.frameid;
}
...
void app_main(void) {
    ...
    {	//frisch initialisieren
    	...
    	rtc_cycles = 0;
    	rtc_no_response_cnt = 0;
    	rtc_sleep_time_ms = DEFAULT_SLEEP_TIME_MS;
    	rtc_tx_pwr = MAX_TX_POWER;
